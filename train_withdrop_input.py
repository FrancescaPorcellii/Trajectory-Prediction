...
def masked_smooth_l1(pred, target, mask):
    ...
    # se nessun punto valido → restituisci 0 (o qualunque costo fisso)
    if mask.sum() == 0:
  ...
  if mode == 'save':
    for epoch in range(num_epochs):
        ...
        for batch in dataloader:
            ...
            optimizer.zero_grad()
            pred = model(x)
            loss = criterion(pred, y)      # << uso la loss mascherata
            loss.backward()
            optimizer.step()
    ...
    # ----------------------------
    torch.save(model.state_dict(), 'model_weights_drop_input.pth')
  elif mode == 'load':
    ...
    model.load_state_dict(torch.load('model_weights_drop_input.pth'))
